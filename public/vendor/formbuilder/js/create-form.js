jQuery(function () {
    $('#form_type').on('change', function () {
        ...
        var form_type = $(this).find('option:selected').val();
        var visibility = $('#visibility').find('option:selected').val();

        if (form_type == 1) {
            $("div").remove(".form_action_edit");
            $(".return_div").show();
            formBuilder = fbEditor.formBuilder({dataType: 'json'})
    ...
    $('#visibility').change(function (e) {
        e.preventDefault()
        var form_type = $('#form_type').find('option:selected').val();
        var ref = $(this)
    ...
    var fbOptions = {
        ...
        disableFields: ['button', // buttons are not needed since we are the one handling the submission
        ],  // field types that should not be shown
        disabledAttrs: [// 'access',
        ],
        typeUserDisabledAttrs: {
            'file': ['multiple', 'subtype',], 'checkbox-group': ['other',],
        },
    ...
    if (form_type == 1) {
        ...
        $(".return_div2").show();
        if ($('#createFormForm').attr('data-form-method')) {
            formBuilder = fbEditor.formBuilder(fbOptions)
        }
    }
    ...
    fbSaveBtn.click(function (e) {
        ...
        if (form_type != 1) {
            ...
            if (!formBuilder.actions.getData().length) {
                swal({
                    title: "Error", text: "The form builder cannot be empty", icon: 'error',
                })
                return
            }
        }
        ...
        sConfirm("Save this form definition?", function () {
            ...
            var postData = {
                ...
                form_url: $('#form_url_edit').val(),
                form_action: $('#form_action_edit').val(),
                form_email: $('#form_email_edit').val(),
            }

            var method = form.data('formMethod') ? 'PUT' : 'POST'
            jQuery.ajax({
                url: form.attr('action'), processData: true, data: postData, method: method, cache: false,
            })
                .then(function (response) {
                    ...
                    if (response.success) {
                        ...
                        swal({
                            title: "Form Saved!", text: response.details || '', icon: 'success',
                        })
                        ...
                        swal({
                            title: "Error", text: response.details || 'Error', icon: 'error',
                        })
                    }
                }, function (error) {
